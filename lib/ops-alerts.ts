...
const SENDER_DOMAINS = (env("SENDER_DOMAINS", "mail.menulift.app") ?? "mail.menulift.app")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const FROM_DOMAIN = SENDER_DOMAINS[0] ?? "mail.menulift.app";
...
export async function sendOpsAlert(args: OpsAlertArgs): Promise<void> {
  ...
  try {
    await resend.emails.send({
      from: `MenuLift Ops <ops@${FROM_DOMAIN}>`,
      to: OPERATOR_EMAIL,
