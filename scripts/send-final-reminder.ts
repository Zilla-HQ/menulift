 ...
 * trust. From + reply-to at jack@mail.menulift.app (no personal-email
 * leakage).
 */
...
async function main() {
  ...
  // ─── Compose ──────────────────────────────────────────────────────
  const subject = `${firstName ? firstName + ", " : ""}your MenuLift discount expires ${hoursLeft.startsWith("0") ? "in less than an hour" : `at ${expiryHuman}`} — no rush`;

  const text = `Hey ${firstName},

Quick midday note — your ${code} discount on the MenuLift Tune-Up is good for ~${hoursLeft} more hours (expires ${expiryHuman}).
...
<p>Quick midday note — your <code style="font-family:ui-monospace,SFMono-Regular,'SF Mono',monospace;background:#fef9c3;padding:2px 6px;border-radius:3px;font-weight:600;">${code}</code> discount on the MenuLift Tune-Up is good for <strong>~${hoursLeft} more hours</strong> (expires ${expiryHuman}).</p>
...
<img src="${sampleAfter}" alt="MenuLift restyled photo of your listing" style="display:block;width:100%;max-width:520px;height:auto;border-radius:10px;border:1px solid #e2e8f0;margin-bottom:18px;" />` : ""}
...
<p>— Jack<br/>Founder, MenuLift<br/><a href="${APP_URL}" style="color:#475569;">${APP_URL.replace(/^https?:\/\//, "")}</a></p>
</body></html>`;

  // ─── Send ─────────────────────────────────────────────────────────
  const result = await resend.emails.send({
    from: `Jack at MenuLift <${FROM_ADDR}>`,
    to: email,
    replyTo: FROM_ADDR,
    subject,
    text,
