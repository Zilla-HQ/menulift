...
export async function POST(req: NextRequest) {
  ...
  const targets = (await db.execute(sql`
    SELECT DISTINCT ON (l.id)
                    ...
                    p.id as preview_id
    FROM menulift.listings l
    JOIN menulift.previews p ON p.listing_id = l.id AND p.service_id = 'photo-staging'
    WHERE l.agent_email IS NOT NULL AND l.agent_email != ''
      ...
      AND NOT EXISTS (
        SELECT 1 FROM menulift.outreach_events e2
        WHERE e2.listing_id = l.id AND e2.template_id = ${TEMPLATE_ID}
      )
      AND NOT EXISTS (
        SELECT 1 FROM menulift.orders o
  ...
  for (const t of targets) {
    ...
    const replyEmail = env("REPLIES_EMAIL", "replies@menulift.app");

    const subject = `${addr} — I'll do this one free in exchange for one line of feedback`;
    const bodyText = `Hey ${firstName},

I'm trying to get my first 5 paid agents on MenuLift and you're one of the people who clicked my email earlier — so here's a straight trade.
...
MenuLift`;
    ...
    const bodyMjml = `<mjml><mj-body background-color="#f4f5f7">
      ...
      <mj-section background-color="#ffffff" padding="32px 32px 8px"><mj-column>
        ...
        <mj-text font-size="16px" line-height="1.6">I'm trying to get my first 5 paid agents on MenuLift and you're one of the people who clicked my email earlier — so here's a straight trade.</mj-text>
